Follow these instructions across repositories unless a repository-local instruction file defines a meaningful project-specific deviation.
...
- Each repository should keep a slim local instruction file that covers only application purpose, platform or tenant specifics, cloud resource naming, auth claim specifics, approved provider scope, and explicit exceptions to the global standards.
