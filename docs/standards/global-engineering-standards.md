...
## Purpose
This document defines reusable engineering standards intended to apply across repositories and future applications. Repository-local instruction files should keep only project-specific context, constraints, and approved deviations.

## Architecture and Structure
- Keep composition at the application entry point and keep business behavior out of controllers.
- Separate concerns clearly across API, orchestration, configuration, diagnostics, models, and infrastructure.
- Prefer modular components with focused responsibilities over broad utility classes.
- Reuse existing components before introducing new layers.
- Avoid speculative abstractions.

## Coding Style and Design
- Follow repository conventions first, then standard .NET conventions.
- Use descriptive names for classes, methods, files, and tests.
- Prefer small classes and small methods with one clear purpose.
...
## SOLID and Dependency Management
...
- Depend on abstractions only where external dependencies, seams, or testing justify them.
...
- Centralize API error formatting where possible.
- Keep validation close to entry points or dedicated validation services.

## Security
- Secure by default.
- Do not store secrets in code.
- Use least privilege for infrastructure identities and application permissions.
...
## Reuse Guidance
Use this file as the reusable baseline for future repositories. Each repository should keep a slim local instruction file that covers:
- application purpose
